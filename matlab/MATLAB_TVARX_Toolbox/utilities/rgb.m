function out = rgb(in)
% RGB: Translate a colour from multiple formats into matlab colour format.
%
% Syntax:
%
%       out = rgb(in)
...
% Description:
%
%       Translate a colour from multiple formats into matlab colour format.
%
...
% Links:
%
%       translatecolorchar, translatecolorstring, translatecolorRGB, qbcolor, rgb_parse, bytes4, rgb_list, get_cols
%
% References:
%
%       none
...
elseif isa(in,'double') && size(in,1) == 1 && size(in,2) == 3
	...
	warning('rgb:InputError','rgb: Unrecognised color format, black assumed');
	out = [0 0 0];
end
...
switch in
    ...
    case 'c'
        out = [0 1 1];
    otherwise
        warning('rgb:InputError',['rgb:Unrecognised colour "' in '", black assumed'])
        out = [0 0 0];
        return
end

end % subfunction {translatecolorchar}
...
if args.N > 3
    warning('rgb:InputError','rgb: Too many words in color description, any more than 3 will be ignored');
end
...
for n = 1:size(cols,1)
	...
	for m = 1:length(names)
        if strcmp(args.tokens{3},names{m})
            col=cols{n,2};
            break
        end
	end
...
if isempty(col)
	warning('rgb:InputError',['rgb: Unrecognised colour "' args.tokens{3} '", black assumed'])
	out = [0 0 0];
	return
end
...
switch args.tokens{1}
    ...
    case 'extremely'
        fac = 0.125;
    otherwise
        warning('rgb:InputError',['rgb: Unrecognised modifier "' args.tokens{1} '", normal assumed'])
        fac = 0.5;
end
...
switch args.tokens{2}
    ...
    case 'dark'
        out = col*fac;
    otherwise
        warning('rgb:InputError',['rgb: Unrecognised descriptor "' args.tokens{2} '", normal assumed'])
        out = col;
end

end % subfunction {translatecolorstring}
...
end % subfunction {translatecolorRGB}
...
switch in
    ...
    case 7
        rgb = [1 1 1];
    otherwise
        warning('rgb:InputError','rgb:Unrecognised QBasic color, black assumed');
        out = [0 0 0];
        return
end
...
while pos <= l
	...
	if inquotes
		if ch == '"'
			inquotes = 0;
			tokens = {tokens{:} token}; %#ok<CCAT>
		else
			token = [token ch]; %#ok<AGROW>
		end
	elseif intoken
		if ch == ' ' || ch == tab
			intoken = 0;
...
if inquotes
    error(['Unpaired quotes in <' str '>']);
end

tokens = tokens';

end % subfunction {rgb_parse}
...
out = [0 0 0 0];
if in<0 || in>(2^32-1)
	warning('rgb:InputError','rgb:DWORD out of range, zero assumed');
	return
end
