...
function s = bvar
s = BEARsettings(2, 'ExcelFile', 'datab_iv.xls');
...
s.results_sub='bvar_iv';
s.results_path = fullfile(pwd, 'results_iv');
...
% 61=Steady State priors
s.prior=21;
% hyperparameter: autoregressive coefficient (if PriorExcel is equal to 0)
...
s.F=0;            % activate unconditional forecasts (1=yes, 0=no)
s.FEVD=1;         % activate forecast error variance decomposition (1=yes, 0=no)
...
% IV options:
%s.strctident.Instrument='BVI'; % specify Instrument to identfy Shock
%s.strctident.startdateIV='2001q1';
%s.strctident.enddateIV='2021q3';
...
%% Forecast 
s.Fstartdate='2012q1'; % start date for forecasts (has to be an in-sample date; otherwise, ignore and set Fendsmpl=1)
s.Fenddate='2021q3';   % end date for forecasts
...
%% Credibility bands
s.cband=0.95;    % confidence/credibility level for VAR coefficients
s.IRFband=0.95;  % confidence/credibility level for impusle response functions
